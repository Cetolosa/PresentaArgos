...
let instrSelectedNode = null;
let instrJPOpen = false;

function instrSetView(view){
  const left=document.getElementById('instr-preview-left');
  const right=document.getElementById('instr-preview-right');
  const tech=document.getElementById('instr-preview-tech');
  const label=document.getElementById('instr-preview-label');
  const btnL=document.getElementById('instr-btn-left');
  const btnB=document.getElementById('instr-btn-both');
  const btnR=document.getElementById('instr-btn-right');
  const preview=document.getElementById('instr-preview');

  // Reset all
  [left,right,tech].forEach(el=>{if(el)el.style.opacity='0';});
  [btnL,btnB,btnR].forEach(b=>{if(b){b.style.background='transparent';b.style.borderColor='rgba(255,255,255,.1)';b.style.color='rgba(255,255,255,.4)';}});

  if(view==='left'){
    if(left){left.style.opacity='1';}
    if(label)label.textContent='RELATO';
    if(preview)preview.style.background='rgba(255,255,255,.04)';
    if(btnL){btnL.style.background='rgba(255,255,255,.1)';btnL.style.borderColor='rgba(255,255,255,.25)';btnL.style.color='rgba(255,255,255,.8)';}
  } else if(view==='both'){
    if(left){left.style.opacity='1';}
    if(right){right.style.opacity='1';}
    if(label)label.textContent='DÍPTICO';
    if(preview)preview.style.background='rgba(255,255,255,.04)';
    if(btnB){btnB.style.background='rgba(255,255,255,.1)';btnB.style.borderColor='rgba(255,255,255,.25)';btnB.style.color='rgba(255,255,255,.8)';}
  } else {
    if(tech){tech.style.opacity='1';}
    if(label)label.textContent='TÉCNICO';
    if(preview)preview.style.background='rgba(56,189,248,.04)';
    if(btnR){btnR.style.background='rgba(56,189,248,.1)';btnR.style.borderColor='rgba(56,189,248,.3)';btnR.style.color='rgba(56,189,248,.9)';}
  }
}

function instrToggleJP(){
  instrJPOpen=!instrJPOpen;
  const panel=document.getElementById('instr-jp-panel');
  const inner=document.getElementById('instr-jp-inner');
  if(panel){panel.style.width=instrJPOpen?'120px':'0';}
  if(inner){inner.style.opacity=instrJPOpen?'1':'0';}
}

function instrSelectNode(id){
  instrSelectedNode=id;
  const detail=document.getElementById('instr-node-detail');
  const nodeA=document.getElementById('instr-node-a');
  if(detail){detail.style.opacity='1';}
  if(nodeA){nodeA.style.background='rgba(56,189,248,.25)';nodeA.style.borderColor='rgba(56,189,248,.6)';}
}

function showInstructions(){
  hideCover();
  const sc=document.getElementById('instructions-screen');
  if(!sc) return;
  sc.style.display='flex';
  requestAnimationFrame(()=>{ sc.style.opacity='1'; });
  startDipticoAnimation();
}
...
function hideInstructions(){
  ...
  if(!sc) return;
  sc.style.opacity='0';
  setTimeout(()=>{ sc.style.display='none'; stopDipticoAnimation(); }, 500);
}

let _dipticoAnimTimer=null;
let _dipticoState=0;

function startDipticoAnimation(){
  _dipticoState=0;
  _dipticoTick();
}

function stopDipticoAnimation(){
  if(_dipticoAnimTimer) clearTimeout(_dipticoAnimTimer);
}

function _dipticoTick(){
  const left=document.getElementById('instr-left');
  const right=document.getElementById('instr-right');
  const tech=document.getElementById('instr-tech');
  if(!left||!right||!tech) return;

  const active='background:rgba(255,255,255,.12);border:1px solid rgba(255,255,255,.25);border-radius:8px;';
  const inactive='border-radius:8px;';

  const states=[
    [active,'background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.06);border-radius:8px;','background:rgba(255,255,255,.03);border:1px solid rgba(255,255,255,.05);border-radius:8px;'],
    ['background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.06);border-radius:8px;',active,'background:rgba(255,255,255,.03);border:1px solid rgba(255,255,255,.05);border-radius:8px;'],
    ['background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.06);border-radius:8px;','background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.06);border-radius:8px;',active],
  ];

  const s=states[_dipticoState%3];
  // Preserve inner content by only updating background/border via dataset
  left.style.cssText=left.style.cssText.replace(/background:[^;]+;/g,'').replace(/border:[^;]+;/g,'')+s[0];
  right.style.cssText=right.style.cssText.replace(/background:[^;]+;/g,'').replace(/border:[^;]+;/g,'')+s[1];
  tech.style.cssText=tech.style.cssText.replace(/background:[^;]+;/g,'').replace(/border:[^;]+;/g,'')+s[2];

  _dipticoState++;
  _dipticoAnimTimer=setTimeout(_dipticoTick, 1400);
}
...
function resetDiptico(){
  // Reset beat counter only — cards stay visible
  dipticoBeat=0;
  const cards=document.querySelectorAll('#r-text-wrap .meta, #r-text-wrap #r-ins-meta');
  cards.forEach(c=>{c.style.transition='none';c.style.opacity='1';});
}
...
function handleNext(){
  ...
  // Special chapters keep their beats
  if(d.n===9){ advanceRolando(); return; }
  if(d.n===10){ if(!advanceCh10()) go(1); return; }
  if(d.n===11){ if(!advanceCh11()) go(1); return; }

  // All other chapters: cycle through sequence views, then go to next chapter
...
function renderRelatoBeat0(d){
  ...
  const nt=$('narr-text');
  if(!nt) return;
  // Show full content immediately — no beats, no opacity hiding
  const html=markup(d.narrFull||d.narr||'');
  nt.innerHTML='<div id="narr-col1" style="opacity:1"></div><div id="narr-col2" style="opacity:1"></div>';
  const [c1html,c2html]=splitNarr(html);
  nt.querySelector('#narr-col1').innerHTML=c1html;
  nt.querySelector('#narr-col2').innerHTML=c2html||'';
  // NO resetRelato() — columns stay visible
}

function advanceRelato(){
  // No internal beats — always return false so handleNext advances the sequence
  return false;
}
...
function setupTecnoDiagram(){
  ...
  // Show all nodes immediately — no beat animation
  [...wrap.querySelectorAll('[data-nid],.d1-cable-connector,.d1-output-pipe,.d1-log-line,.pipe-line,.pipe-text')].forEach(el=>{
    el.style.transition='none';
    el.style.opacity='1';
  });

  // Select first node for detail panel
  const nodes=[...wrap.querySelectorAll('[data-nid]')];
  if(nodes[0]) selNode(nodes[0].dataset.nid);

  // Show all tecnico narr text
  const tn=$('tecnico-narr');
  if(tn){
    tn.innerHTML=CH1_TEXTS.map(t=>'<div class="tn-item">'+t+'</div>').join('');
...
function advanceTecno(){
  // No internal beats — always return false
  return false;
}
function _advanceTecno_unused(){
